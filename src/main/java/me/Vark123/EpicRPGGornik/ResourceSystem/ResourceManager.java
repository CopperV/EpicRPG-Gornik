...
public final class ResourceManager {
	...
	public Collection<AResource> getCataclysmResources() {
		return resources.stream().filter(resource -> resource instanceof CatResource).collect(Collectors.toSet());
	}
	...
	public Optional<AResource> getResourceByMMId(String mmId) {
		return resources.stream()
				.filter(resource -> resource.getMmId().equals(mmId)).findFirst();
	}
